...
const MAX_STOPPED_CONTINUATIONS = 3;
...
function getStoppedResponse(body: string): {
	stopped: boolean;
	parentMessageId: string | number | null;
} {
	let parentMessageId: string | number | null = null;
	let stopped = false;
	for (const line of body.split("\n")) {
		if (!line.startsWith("data: ")) continue;
		try {
			const data = JSON.parse(line.slice(6)) as Record<string, unknown>;
			if (typeof data.response_message_id === "string" || typeof data.response_message_id === "number") {
				parentMessageId = data.response_message_id;
			}
			if (data.p === "response/status" && data.v === "STOPPED") stopped = true;
		} catch {
			// Ignore incomplete SSE data.
		}
	}
	return { stopped, parentMessageId };
}
...
export class DeepSeekWebClient extends BaseApiClient<DeepSeekWebCredentials> {
	...
	override async sendMessage(params: {
		...
		let body = await this.chatCompletions({
		...
		let responseText = await streamToText(body);
		let stoppedResponse = getStoppedResponse(responseText);

		for (let attempt = 0; stoppedResponse.stopped && attempt < MAX_STOPPED_CONTINUATIONS; attempt++) {
			if (stoppedResponse.parentMessageId === null) {
				throw new Error("DeepSeek stopped without a response message id");
			}
			body = await this.chatCompletions({
				sessionId: session.chat_session_id,
				parentMessageId: stoppedResponse.parentMessageId,
				message: "",
				model: params.model,
				preempt: true,
				signal: params.signal,
			...
			const continuationText = await streamToText(body);
			responseText += continuationText;
			stoppedResponse = getStoppedResponse(continuationText);
		}

		if (stoppedResponse.stopped) {
			throw new Error(`DeepSeek stopped after ${MAX_STOPPED_CONTINUATIONS} continuations`);
		}
